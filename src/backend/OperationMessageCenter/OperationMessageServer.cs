...
namespace Log4Pro.CoreComponents.OperationMessageCenter
{
	...
	/// </summary>
	public class OperationMessageServer
	{
		...
		public OperationMessageServer(IConfiguration configuration, IServiceScopeFactory scopeFactory, IServiceProvider serviceProvider, Cache cache)
        {
		...
		{
			var config = configuration.GetSection(OperationMessageServer.APPSETTINGS_SECTION_NAME).Get<OperationMessageCenterAppSettings>();
			if (config == null)
			{
			...
			{
				config.UsedConnectionString = OperationMessageServer.DEFAULT_CONNECTIONSTRING;
			}
			return config;
		}
